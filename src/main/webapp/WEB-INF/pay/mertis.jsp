    ...
    <div data-options="region:'center',border:false">
        ...
        <div id="wu-toolbar">
            ...
            <div class="wu-toolbar-search">
                ...
                <%--<label>百分比：</label><input id="percentage" class="wu-text" style="width:100px">
...
<div id="wu-dialog" class="easyui-dialog" data-options="closed:true,iconCls:'icon-save'"
    ...
    <form id="wu-form" method="post">
        <table>
            <tr>
                <td width="60" align="right">绩效百分比:</td>
                <td>
                    <input type="hidden" name="mertisMoneyId" class="wu-text"/>
                    <input id="mertispercontage" type="text" name="percontage" class="wu-text" readonly="readonly"/>
                </td>
            </tr>
            <tr>
                <td align="right">绩效工资:</td>
                <td>
                    <input type="text" name="mertisMoneyNum" class="wu-text"/>
                </td>
            </tr>
        </table>
    </form>
...
<script type="text/javascript">
    ...
    //     var url='getAllMertisMonteyBySql.mertis?percentage='+percentage;
    //     console.log(url);
    //     openthe(url);
    // }
    ...
    function edit() {
        ...
        $('#wu-form').form('submit', {
            url: 'editMertisMoneyByMertisMoneyId.mertis',
            success: function (data) {
                var data = eval('(' + data + ')');
                if (data) {
                    $.messager.alert('信息提示', '提交成功！', 'info');
        ...
        $.messager.confirm('信息提示', '确定要删除该记录？', function (result) {
            if (result) {
                ...
                $(items).each(function () {
                    ...
                    ids += this.mertisMoneyId + ","
                });
                console.log(ids);

                $.ajax({
                    url: 'removeMertisMoneyBatch.mertis',
                    data: {id: ids},
                    success: function (data) {
                        if (data) {
    ...
    function openAdd() {
        $('#wu-form').form('clear');
        $("#mertispercontage").prop("readonly", false);
        $('#wu-dialog').dialog({
            closed: false,
            modal: true,
            title: "添加信息",
    ...
    function openEdit() {
        ...
        $('#wu-form').form('clear');
        $("#mertispercontage").prop("readonly", true);
     ...
     */
    openthe('getAllMertisMonteyBySql.mertis');
    function openthe(url) {
        $('#wu-datagrid').datagrid({
            ...
            columns: [[
                {checkbox: true},
                {field: 'percontage', title: '完成绩效百分比', width: 100},

                {field: 'mertisMoneyNum', title: '绩效工资', width: 100}
